...
if __name__ == "__main__":
    ...
    max_episodes = 5000  # Total episodes (default: 6000)
    ...
    path = "./models/MATD3"
    filename = f"{key}_MATD3_trained_agent.pt"
    ...
    for i,agent in enumerate(pop):
        plt.figure()
        plt.plot(agent.fitness, label='Fitness')
        plt.plot(agent.scores, label='Scores')
        plt.title(f'{i} Agent Scores and Fitness Over Episodes')
        plt.xlabel('Episode')
        plt.legend()
        plt.grid(True)
        plt.savefig(f'{path}/{key}_agent_fitness_{i}.png')  # Save the figure